                    ...
                    <?php endif; ?>
                      <li class="active sub-menu">
                        ...
                        <ul>
                            ...
                            <li><a class="active" href="mouvement_eleves.php">Mouvement des élèves </a></li>
                        </ul>
                    </li>
<?php if(isset($_SESSION['current_session']) && $_SESSION['current_session'] == "admin"): ?>
                     <li class=" sub-menu">
...
"></i>Affichage des résultats</a>
                        ...
                        <div id="todo1" class="card">
                            <div class="card-header bgm-bluegray1 ch-alt m-b-20">
                               ...
                               <?php if(isset($_SESSION['paroisse'])):
                                    ...
                                    echo "ENREGISTREMENT DU MOUVEVEMENT D'ELEVES DANS LA PAROISSE ".$np[0]["nom_paroisse"];
                                ?>
                                <?php else: 
                                    echo "ENREGISTREMENT DU MOUVEVEMENT D'ELEVES DANS TOUTES LES PAROISSES";
        ...
        foreach ($table_option1 as $option1) {
            ...
            $class_sec_co = query("SELECT COUNT(DISTINCT belongs_to) as nbr FROM nouveau_inscrit WHERE belongs_to = ?",$id_e);
            ...
            if ($class_sec_co[0]["nbr"] == 0) {
                echo("<option  value='$nom_ecole**$niveau**$nom_p**$nom_sous_d'>" .$nom_ecole ."</option>");
                                    ...
                                    <div class="col-sm-2">
                                        <button id="select_ecole_btn_mouv" class="btn btn-primary">Confirmer</button>
                                    </div>
        ...
        echo '
                                ...
                                <div class="modal-header1 ">
                                            <h4 class="modal-title p-l-20">Modifier le mouvement des élèves</h4>
                                        </div>
...
<select id="select_ecole_mod" data-live-search="true" class="selectpicker"  title="Selectionner une école" data-selected-text-format="count" tabindex="-98"  >';
                                    ...
                                    {  
                                        ...
                                        if (count($coord) >= 1) {
                                            foreach ($coord as $res ) {
                                               $id = $res["id"];
                                                $class_sec_co = query("SELECT COUNT(DISTINCT belongs_to) as nbr FROM nouveau_inscrit WHERE belongs_to = ?",$id);
            ...
            if ($class_sec_co[0]["nbr"] == 0) {
                                     ...
                                     echo '</select></div>
                                     <div class="col-sm-2">
                                     <button class="btn btn-primary" id="modif_btn_mouvement">Confirmer</button>
                                     </div>
                                     <div class="row">
    ...
    ?>
            ...
            <form id="ecolepri_form_mouv"  class="row p-10" style="display: none;">
...
echo "<tr id='tabp_mouv$i'>";
    ...
    if ( $j==0 || $j==1 || $j==3 || $j==4 || $j==6 ||$j==7 ||$j==9|| $j==10|| $j==12||$j==13 ) {
     echo "<td><input autocomplete='false' id='tabp_mouv$i$j' type='text' class='form-control tabp_mouv tabp_mouvfg$i$j' name='tabp_mouvfg$i$j'></td>";
    }
