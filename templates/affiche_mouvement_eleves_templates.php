...
"></i>Affichage des résultats</a>
                        ...
                        <ul>
                            ...
                            <li><a class="active" href="affiche_mouvement_eleves.php">Affiche mouvement des élèves </a></li>
                            <li><a class="" href="affiche_age_eleves.php">Affiche ages des élèves</a></li>
                            <li><a class="" href="affiche_redoublant_mouvement_eleves.php">Affiche des rédoublants</a></li>
        ...
        <span class='titrelogo'> $titre</span>
            <span class='center_naledi'>$bp <br><br></span></div></div>";
                        ...
                        <span class='titre_print text-uppercase'>Affiche mouvement des élèves</span><br>
        ...
        foreach ($id_last_insert as $ecole_) {
            ...
            $class_nouv_in = query("SELECT * FROM nouveau_inscrit WHERE belongs_to = ? ",$ecole_["id"]);
            $class_redoublants = query("SELECT * from `redoublants` WHERE belongs_to = ? ",$ecole_["id"]);
            $class_redoublantsf = query("SELECT sum(`effectif_F`) as som from `redoublants` WHERE belongs_to = ? ",$ecole_["id"]);
            $class_redoublantsg = query("SELECT sum(`effectif_G`) as som from `redoublants` WHERE belongs_to = ? ",$ecole_["id"]);

            $class_promu1 = query("SELECT * FROM promus WHERE belongs_to = ?",$ecole_["id"]);
            $class_transferer1 = query("SELECT * FROM transfere WHERE belongs_to = ?",$ecole_["id"]);
            $class_abandon1 = query("SELECT * FROM abandon WHERE belongs_to = ?",$ecole_["id"]);
            $class_certifie1 = query("SELECT * FROM certifie_diplome WHERE belongs_to = ?",$ecole_["id"]);

            if ($ecole_["id_niveau"] == "Maternelle" ) {
                if ( count($class_nouv_in) > 0) {
                    $NOUV_INSCRIT_mat_f= $NOUV_INSCRIT_mat_f + $class_nouv_in["0"]["effectif_F"];
                    $NOUV_INSCRIT_mat_g= $NOUV_INSCRIT_mat_g + $class_nouv_in["0"]["effectif_G"];
                }
                if (count($class_redoublants) > 0) {
                    $REDOUBLANTS_mat_f= $REDOUBLANTS_mat_f + $class_redoublantsf[0]["som"];
                    $REDOUBLANTS_mat_g= $REDOUBLANTS_mat_g + $class_redoublantsg[0]["som"];
                }
                if (count($class_promu1) > 0) {
                    $PROMUS_mat_f= $PROMUS_mat_f + $class_promu1[0]["effectif_F"];
                    $PROMUS_mat_g= $PROMUS_mat_g + $class_promu1[0]["effectif_G"];
                }
                if (count($class_transferer1) > 0) {
                    $TRANSF_mat_f= $TRANSF_mat_f + $class_transferer1[0]["effectif_F"];
                    $TRANSF_mat_g= $TRANSF_mat_g + $class_transferer1[0]["effectif_G"];
                }
                if (count($class_abandon1) > 0) {
                    $ABANDONS_mat_f= $ABANDONS_mat_f + $class_abandon1[0]["effectif_F"];
                    $ABANDONS_mat_g= $ABANDONS_mat_g + $class_abandon1[0]["effectif_G"];
                }
                if (count($class_certifie1) > 0) {
                    $CERTIF_DIPL_mat_f= $CERTIF_DIPL_mat_f + $class_certifie1[0]["effectif_F"];
                    $CERTIF_DIPL_mat_g= $CERTIF_DIPL_mat_g + $class_certifie1[0]["effectif_G"];
                }
            ...
            elseif ($ecole_["id_niveau"] == "Primaire") {
                if ( count($class_nouv_in) > 0) {
                    $NOUV_INSCRIT_pri_f= $NOUV_INSCRIT_pri_f + $class_nouv_in["0"]["effectif_F"];
                    $NOUV_INSCRIT_pri_g= $NOUV_INSCRIT_pri_g + $class_nouv_in["0"]["effectif_G"];
                }
                if (count($class_redoublants) > 0) {
                    $REDOUBLANTS_pri_f= $REDOUBLANTS_pri_f + $class_redoublantsf[0]["som"];
                    $REDOUBLANTS_pri_g= $REDOUBLANTS_pri_g + $class_redoublantsg[0]["som"];
                }
                if (count($class_promu1) > 0) {
                    $PROMUS_pri_f= $PROMUS_pri_f + $class_promu1[0]["effectif_F"];
                    $PROMUS_pri_g= $PROMUS_pri_g + $class_promu1[0]["effectif_G"];
                }
                if (count($class_transferer1) > 0) {
                    $TRANSF_pri_f= $TRANSF_pri_f + $class_transferer1[0]["effectif_F"];
                    $TRANSF_pri_g= $TRANSF_pri_g + $class_transferer1[0]["effectif_G"];
                }
                if (count($class_abandon1) > 0) {
                    $ABANDONS_pri_f= $ABANDONS_pri_f + $class_abandon1[0]["effectif_F"];
                    $ABANDONS_pri_g= $ABANDONS_pri_g + $class_abandon1[0]["effectif_G"];
                }
                if (count($class_certifie1) > 0) {
                    $CERTIF_DIPL_pri_f= $CERTIF_DIPL_pri_f + $class_certifie1[0]["effectif_F"];
                    $CERTIF_DIPL_pri_g= $CERTIF_DIPL_pri_g + $class_certifie1[0]["effectif_G"];
                }
               
            }
            elseif ($ecole_["id_niveau"] == "Secondaire") {
                if ( count($class_nouv_in) > 0) {
                    $NOUV_INSCRIT_sec_f= $NOUV_INSCRIT_sec_f + $class_nouv_in["0"]["effectif_F"];
                    $NOUV_INSCRIT_sec_g= $NOUV_INSCRIT_sec_g + $class_nouv_in["0"]["effectif_G"];
                }
                if (count($class_redoublants) > 0) {
                    $REDOUBLANTS_sec_f= $REDOUBLANTS_sec_f + $class_redoublantsf[0]["som"];
                    $REDOUBLANTS_sec_g= $REDOUBLANTS_sec_g + $class_redoublantsg[0]["som"];
                }
                if (count($class_promu1) > 0) {
                    $PROMUS_sec_f= $PROMUS_sec_f + $class_promu1[0]["effectif_F"];
                    $PROMUS_sec_g= $PROMUS_sec_g + $class_promu1[0]["effectif_G"];
                }
                if (count($class_transferer1) > 0) {
                    $TRANSF_sec_f= $TRANSF_sec_f + $class_transferer1[0]["effectif_F"];
                    $TRANSF_sec_g= $TRANSF_sec_g + $class_transferer1[0]["effectif_G"];
                }
                if (count($class_abandon1) > 0) {
                    $ABANDONS_sec_f= $ABANDONS_sec_f + $class_abandon1[0]["effectif_F"];
                    $ABANDONS_sec_g= $ABANDONS_sec_g + $class_abandon1[0]["effectif_G"];
                }
                if (count($class_certifie1) > 0) {
                    $CERTIF_DIPL_sec_f= $CERTIF_DIPL_sec_f + $class_certifie1[0]["effectif_F"];
                    $CERTIF_DIPL_sec_g= $CERTIF_DIPL_sec_g + $class_certifie1[0]["effectif_G"];
                }
        ...
        echo "<td>$NOUV_INSCRIT_mat_f </td>";
        echo "<td>$NOUV_INSCRIT_mat_g </td>";
        echo "<td>".($NOUV_INSCRIT_mat_f+$NOUV_INSCRIT_mat_g)."</td>";
        ...
        echo "<td class='tr'>".($NOUV_INSCRIT_mat_f+$REDOUBLANTS_mat_f+$PROMUS_mat_f+$TRANSF_mat_f)."</td>";
        echo "<td class='tr'>".($NOUV_INSCRIT_mat_g+$REDOUBLANTS_mat_g+$PROMUS_mat_g+$TRANSF_mat_g)."</td>";
        
        echo "<td class='tr'>".(($NOUV_INSCRIT_mat_f+$REDOUBLANTS_mat_f+$PROMUS_mat_f+$TRANSF_mat_f)+$NOUV_INSCRIT_mat_g+$REDOUBLANTS_mat_g+$PROMUS_mat_g+$TRANSF_mat_g)."</td>";
        ...
        echo "<td rowspan='1'>Primaire</td>";
         ...
         echo "<td>$NOUV_INSCRIT_pri_f </td>";
        echo "<td>$NOUV_INSCRIT_pri_g </td>";
        echo "<td>".($NOUV_INSCRIT_pri_f+$NOUV_INSCRIT_pri_g)."</td>";
        ...
        echo "<td class='tr'>".($NOUV_INSCRIT_pri_f+$REDOUBLANTS_pri_f+$PROMUS_pri_f+$TRANSF_pri_f)."</td>";
        echo "<td class='tr'>".($NOUV_INSCRIT_pri_g+$REDOUBLANTS_pri_g+$PROMUS_pri_g+$TRANSF_pri_g)."</td>";
        
        echo "<td class='tr'>".(($NOUV_INSCRIT_pri_f+$REDOUBLANTS_pri_f+$PROMUS_pri_f+$TRANSF_pri_f)+$NOUV_INSCRIT_pri_g+$REDOUBLANTS_pri_g+$PROMUS_pri_g+$TRANSF_pri_g)."</td>";
        ...
        echo "<td>$NOUV_INSCRIT_sec_f </td>";
        echo "<td>$NOUV_INSCRIT_sec_g </td>";
        echo "<td>".($NOUV_INSCRIT_sec_f+$NOUV_INSCRIT_sec_g)."</td>";
       ...
       /* for ($i=0; $i < 3 ; $i++) { 
        ...
        echo "<td class='tr'>".($NOUV_INSCRIT_sec_f+$REDOUBLANTS_sec_f+$PROMUS_sec_f+$TRANSF_sec_f)."</td>";
        echo "<td class='tr'>".($NOUV_INSCRIT_sec_g+$REDOUBLANTS_sec_g+$PROMUS_sec_g+$TRANSF_sec_g)."</td>";
        
        echo "<td class='tr'>".(($NOUV_INSCRIT_sec_f+$REDOUBLANTS_sec_f+$PROMUS_sec_f+$TRANSF_sec_f)+$NOUV_INSCRIT_sec_g+$REDOUBLANTS_sec_g+$PROMUS_sec_g+$TRANSF_sec_g)."</td>";
        ...
        $NOUV_INSCRIT_mat_f_tot= $NOUV_INSCRIT_mat_f_tot + $NOUV_INSCRIT_mat_f;
        ...
        $NOUV_INSCRIT_pri_f_tot= $NOUV_INSCRIT_pri_f_tot + $NOUV_INSCRIT_pri_f;
        ...
        $NOUV_INSCRIT_sec_f_tot= $NOUV_INSCRIT_sec_f_tot + $NOUV_INSCRIT_sec_f;
        ...
        $NOUV_INSCRIT_mat_g_tot= $NOUV_INSCRIT_mat_g_tot + $NOUV_INSCRIT_mat_g;
        ...
        $NOUV_INSCRIT_pri_g_tot= $NOUV_INSCRIT_pri_g_tot + $NOUV_INSCRIT_pri_g;
        ...
        $NOUV_INSCRIT_sec_g_tot= $NOUV_INSCRIT_sec_g_tot + $NOUV_INSCRIT_sec_g;
        ...
        echo "<td rowspan='1'>Maternelle</td>";
        echo "<td>$NOUV_INSCRIT_mat_f_tot </td>";
        echo "<td>$NOUV_INSCRIT_mat_g_tot </td>";
        echo "<td>".($NOUV_INSCRIT_mat_f_tot+$NOUV_INSCRIT_mat_g_tot)."</td>";
        ...
        echo "<td class='tr'>".($NOUV_INSCRIT_mat_f_tot+$REDOUBLANTS_mat_f_tot+$PROMUS_mat_f_tot+$TRANSF_mat_f_tot)."</td>";
        echo "<td class='tr'>".($NOUV_INSCRIT_mat_g_tot+$REDOUBLANTS_mat_g_tot+$PROMUS_mat_g_tot+$TRANSF_mat_g_tot)."</td>";
        
        echo "<td class='tr'>".(($NOUV_INSCRIT_mat_f_tot+$REDOUBLANTS_mat_f_tot+$PROMUS_mat_f_tot+$TRANSF_mat_f_tot)+
            $NOUV_INSCRIT_mat_g_tot+$REDOUBLANTS_mat_g_tot+$PROMUS_mat_g_tot+$TRANSF_mat_g_tot)."</td>";
        ...
        echo "<tr class='tr'>";   
         ...
         echo "<td rowspan='1'>Primaire</td>";
        echo "<td>$NOUV_INSCRIT_pri_f_tot </td>";
        echo "<td>$NOUV_INSCRIT_pri_g_tot </td>";
        echo "<td>".($NOUV_INSCRIT_pri_f_tot+$NOUV_INSCRIT_pri_g_tot)."</td>";
        ...
        echo "<td class='tr'>".($NOUV_INSCRIT_pri_f_tot+$REDOUBLANTS_pri_f_tot+$PROMUS_pri_f_tot+$TRANSF_pri_f_tot)."</td>";
        echo "<td class='tr'>".($NOUV_INSCRIT_pri_g_tot+$REDOUBLANTS_pri_g_tot+$PROMUS_pri_g_tot+$TRANSF_pri_g_tot)."</td>";
        
        echo "<td class='tr'>".(($NOUV_INSCRIT_pri_f_tot+$REDOUBLANTS_pri_f_tot+$PROMUS_pri_f_tot+$TRANSF_pri_f_tot)+$NOUV_INSCRIT_pri_g_tot+$REDOUBLANTS_pri_g_tot+$PROMUS_pri_g_tot+$TRANSF_pri_g_tot)."</td>";
        ...
        echo "<td>$NOUV_INSCRIT_sec_f_tot </td>";
        echo "<td>$NOUV_INSCRIT_sec_g_tot </td>";
        echo "<td>".($NOUV_INSCRIT_sec_f_tot+$NOUV_INSCRIT_sec_g_tot)."</td>";
        ...
        echo "<td class='tr'>".($NOUV_INSCRIT_sec_f_tot+$REDOUBLANTS_sec_f_tot+$PROMUS_sec_f_tot+$TRANSF_sec_f_tot)."</td>";
        echo "<td class='tr'>".($NOUV_INSCRIT_sec_g_tot+$REDOUBLANTS_sec_g_tot+$PROMUS_sec_g_tot+$TRANSF_sec_g_tot)."</td>";
        
        echo "<td class='tr'>".(($NOUV_INSCRIT_sec_f_tot+$REDOUBLANTS_sec_f_tot+$PROMUS_sec_f_tot+$TRANSF_sec_f_tot)+$NOUV_INSCRIT_sec_g_tot+$REDOUBLANTS_sec_g_tot+$PROMUS_sec_g_tot+$TRANSF_sec_g_tot)."</td>";
       ...
       /* for ($i=0; $i < 18 ; $i++) { 
        ...
        echo "<td>".($NOUV_INSCRIT_mat_f_tot + $NOUV_INSCRIT_pri_f_tot + $NOUV_INSCRIT_sec_f_tot)."</td>";
        echo "<td>".($NOUV_INSCRIT_mat_g_tot + $NOUV_INSCRIT_pri_g_tot + $NOUV_INSCRIT_sec_g_tot)."</td>";
        echo "<td>".($NOUV_INSCRIT_mat_f_tot + $NOUV_INSCRIT_pri_f_tot + $NOUV_INSCRIT_sec_f_tot + $NOUV_INSCRIT_mat_g_tot + $NOUV_INSCRIT_pri_g_tot + $NOUV_INSCRIT_sec_g_tot)."</td>";
        ...
        echo "<td class='tr'>".($NOUV_INSCRIT_mat_f_tot+$REDOUBLANTS_mat_f_tot+$PROMUS_mat_f_tot+$TRANSF_mat_f_tot+ $NOUV_INSCRIT_pri_f_tot+$REDOUBLANTS_pri_f_tot+$PROMUS_pri_f_tot+$TRANSF_pri_f_tot+$NOUV_INSCRIT_sec_f_tot+$REDOUBLANTS_sec_f_tot+$PROMUS_sec_f_tot+$TRANSF_sec_f_tot)."</td>";

        echo "<td class='tr'>".($NOUV_INSCRIT_mat_g_tot+$REDOUBLANTS_mat_g_tot+$PROMUS_mat_g_tot+$TRANSF_mat_g_tot+ $NOUV_INSCRIT_pri_g_tot+$REDOUBLANTS_pri_g_tot+$PROMUS_pri_g_tot+$TRANSF_pri_g_tot +$NOUV_INSCRIT_sec_g_tot+$REDOUBLANTS_sec_g_tot+$PROMUS_sec_g_tot+$TRANSF_sec_g_tot)."</td>";
        
        echo "<td class='tr'>".((($NOUV_INSCRIT_mat_f_tot+$REDOUBLANTS_mat_f_tot+$PROMUS_mat_f_tot+$TRANSF_mat_f_tot + $NOUV_INSCRIT_pri_f_tot+$REDOUBLANTS_pri_f_tot+$PROMUS_pri_f_tot+$TRANSF_pri_f_tot +$NOUV_INSCRIT_sec_f_tot+$REDOUBLANTS_sec_f_tot+$PROMUS_sec_f_tot+$TRANSF_sec_f_tot))+$NOUV_INSCRIT_mat_g_tot+$REDOUBLANTS_mat_g_tot+$PROMUS_mat_g_tot+$TRANSF_mat_g_tot+ $NOUV_INSCRIT_pri_g_tot+$REDOUBLANTS_pri_g_tot+$PROMUS_pri_g_tot+$TRANSF_pri_g_tot+$NOUV_INSCRIT_sec_g_tot+$REDOUBLANTS_sec_g_tot+$PROMUS_sec_g_tot+$TRANSF_sec_g_tot)."</td>";
